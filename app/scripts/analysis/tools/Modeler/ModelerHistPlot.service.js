"use strict";
/*
  @name:
  @type: service
  @desc: Service to dynamically plot a histogram

*/

let BaseService = require("scripts/BaseClasses/BaseService.coffee");

// let boundMethodCheck = function (instance, Constructor) {
//   if (!(instance instanceof Constructor)) {
//     throw new Error("Bound instance method accessed before binding");
//   }
// };

let ModelerHist = class ModelerHist extends BaseService {
  constructor() {
    super(...arguments);
    this.drawModelCurve = this.drawModelCurve.bind(this);
  }

  initialize() {
    this.kernel = this.app_analysis_modeler_kernel_density_plotter;
    this.gauss = this.app_analysis_modeler_getParams;
    this.bandwith = 4;
    this.kde = null;
    this.median = null;
    this.mean = null;
    this.graphData = null;
    this.elem = null;
    return (this.bins = 5);
  }

  getMedian(arr) {
    let half;
    arr.sort(function (a, b) {
      return a - b;
    });
    half = Math.floor(arr.length / 2);
    if (arr.length % 2) {
      return arr[half];
    } else {
      return (arr[half - 1] + arr[half]) / 2.0;
    }
  }

  getMean(arr) {
    let a, j, len, sum;
    sum = 0;
    for (j = 0, len = arr.length; j < len; j++) {
      a = arr[j];
      sum += a;
    }
    return (sum / arr.length).toFixed(2);
  }

  plotHist( // def will be called by drawHist()
    bins,
    container,
    arr,
    _graph,
    gdata,
    x,
    height,
    width,
    data,
    median,
    mean,
    bounds
  ) {
    let a,
      bar,
      dataHist,
      dataSetYMax,
      getColor,
      j,
      len,
      padding,
      rect_width,
      sizeOfData,
      stats,
      tooltip,
      xAxis,
      xLength,
      xMean,
      xMedian,
      y,
      yAxis,
      yMax;
    //    tooltip
    $("#tooltip").remove();
    $("#slidertext").remove();
    container
      .append("text")
      .attr("id", "slidertext")
      .text("Bins: " + bins)
      .attr("position", "relative")
      .attr("left", "50px");
    dataHist = d3.layout.histogram().frequency(false).bins(bins)(arr);
    sizeOfData = arr.length;
    //#console.log dataHist #array for each bin, each array has all data points
    //create array of objects that store mean and median of each set
    stats = [];
    for (j = 0, len = dataHist.length; j < len; j++) {
      a = dataHist[j];
      stats.push({
        mean: this.getMean(a),
        median: this.getMedian(a),
      });
    }
    //#console.log stats
    xMean = function (d, i) {
      return stats[i].mean;
    };
    xMedian = function (d, i) {
      return stats[i].median;
    };
    xLength = function (d, i) {
      return dataHist[i].length;
    };
    _graph.selectAll("g").remove();
    _graph.select(".x axis").remove();
    _graph.select(".y axis").remove();
    padding = 50;
    x = d3.scale.linear().range([padding, width - padding]);
    y = d3.scale.linear().range([height - padding, padding]);
    //need to change x and y ranges
    dataSetYMax =
      d3.max(
        dataHist.map(function (i) {
          return i.length;
        })
      ) / sizeOfData;
    yMax = Math.max(dataSetYMax, bounds.yMax);
    x.domain([
      d3.min(data, function (d) {
        return parseFloat(d.x);
      }),
      d3.max(data, function (d) {
        return parseFloat(d.x);
      }),
    ]);
    y.domain([0, yMax]);
    yAxis = d3.svg.axis().scale(y).orient("left");
    xAxis = d3.svg.axis().scale(x).orient("bottom");
    getColor = d3.scale.category10();
    // add the tooltip area to the webpage
    tooltip = container
      .append("div")
      .attr("class", "tooltip")
      .attr("id", "tooltip");
    // x axis
    _graph
      .append("g")
      .attr("class", "x axis")
      .attr("transform", "translate(0," + (height - padding) + ")")
      .call(xAxis)
      .style("font-size", "16px");
    // y axis
    _graph
      .append("g")
      .attr("class", "y axis")
      .attr("transform", "translate(" + padding + ",0)")
      .call(yAxis)
      .style("font-size", "16px");
    // make x y axis thin
    _graph.selectAll(".x.axis path").style({
      fill: "none",
      stroke: "black",
      "shape-rendering": "crispEdges",
      "stroke-width": "1px",
    });
    _graph.selectAll(".y.axis path").style({
      fill: "none",
      stroke: "black",
      "shape-rendering": "crispEdges",
      "stroke-width": "1px",
    });
    // rotate text on x axis
    _graph
      .selectAll(".x.axis text")
      .attr("transform", function (d) {
        return (
          "translate(" +
          this.getBBox().height * -2 +
          "," +
          this.getBBox().height +
          ")rotate(-40)"
        );
      })
      .style("font-size", "16px");
    // Title on x-axis
    _graph
      .append("text")
      .attr("class", "label")
      .attr("text-anchor", "middle")
      .attr(
        "transform",
        "translate(" + width + "," + (height - padding / 2) + ")"
      )
      .text(gdata.xLab.value);
    // Title on y-axis
    _graph
      .append("text")
      .attr("class", "label")
      .attr("text-anchor", "middle")
      .attr("transform", "translate(0," + padding / 2 + ")")
      .text("Counts");
    // bar elements
    bar = _graph.selectAll(".bar").data(dataHist);
    bar.enter().append("g");
    rect_width = (width - 2 * padding) / bins;
    bar
      .append("rect")
      .attr("x", function (d) {
        return x(d.x);
      })
      .attr("y", function (d) {
        return y(d.y);
      })
      .attr("width", rect_width)
      .attr("height", function (d) {
        return Math.abs(height - y(d.y)) - padding;
      })
      .attr("stroke", "white")
      .attr("stroke-width", 1)
      .style("fill", getColor(0))
      .on("mouseover", function (d, i) {
        d3.select(this).transition().style("fill", getColor(1));
        tooltip.transition().duration(200).style("opacity", 0.9);
        return tooltip
          .html(
            '<div style="background-color:white; padding:5px; border-radius: 5px">' +
              gdata.xLab.value +
              "</br>" +
              "Median: " +
              xMedian(d, i) +
              "</br>" +
              "Mean: " +
              xMean(d, i) +
              "</br>" +
              "N: " +
              xLength(d, i) +
              "</div>"
          )
          .attr("value", stats[i].mean)
          .style("display", "block")
          .style("opacity", 0.4)
          .style("padding", 2)
          .style("border", 0)
          .style("border-radius", 8)
          .style("left", d3.select(this).attr("x") + "px")
          .style("top", d3.select(this).attr("y") + "px");
        //          .text("Mean1: "+ stats[i].mean + '</br>'+ "Median: " + stats[i].median)

        //        tooltip.append("p")
        //          .text("Mean: "+ stats[i].mean)
        //          .append("br")
        //          .append("p")
        //          .text("Median: "+ stats[i].median)

        //        tooltip.append("/br")
      })
      .on("mouseout", function () {
        d3.select(this).transition().style("fill", getColor(0));
        return tooltip.style("display", "none");
      });
    return bar
      .append("text")
      .attr("x", function (d) {
        return x(d.x);
      })
      .attr("y", function (d) {
        return y(d.y) - 25;
      })
      .attr("dx", function (d) {
        return 0.5 * rect_width;
      })
      .attr("dy", "20px")
      .attr("fill", "black")
      .attr("text-anchor", "middle")
      .attr("z-index", 1)
      .text(function (d) {
        return d3.format(".2f")(d.y);
      });
  }

  drawModelCurve(
    graphData = this.graphData,
    _graph,
    elem = this.elem,
    container,
    labels,
    width,
    height,
    ranges,
    modelBounds
  ) {
    let bottomBound,
      curveData,
      leftBound,
      lineGen,
      modelData,
      padding,
      rightBound,
      top,
      topBound,
      xAxis,
      xScale,
      yAxis,
      yScale;
    // boundMethodCheck(this, ModelerHist);
    this.graphData = graphData;
    this.elem = elem;
    modelData = graphData.modelData;
    console.log("Plotting Model Data");
    if (modelData) {
      container = d3.select(elem[0]);
      container.selectAll("path").remove();
      leftBound = modelData.stats.stats.leftBound;
      rightBound = modelData.stats.stats.rightBound;
      topBound = modelData.stats.stats.topBound;
      bottomBound = modelData.stats.stats.bottomBound;
      curveData = modelData;
      padding = 50;
      //xScale = d3.scale.linear().range([0, width]).domain([modelData.xMin, modelData.xMax])
      //yScale = d3.scale.linear().range([height-padding, 0]).domain([bottomBound, topBound])

      //************** Change for quantile scaling **************
      xScale = d3.scale
        .linear()
        .range([padding, width - padding])
        .domain([modelData.stats.xMin, modelData.stats.xMax]);
      //changin scale for larger range
      //xScale = d3.scale.linear().range([padding, width - padding ]).domain([modelData.stats.leftBound, modelData.stats.rightBound])
      top = Math.max(modelData.yMax, topBound);
      yScale = d3.scale
        .linear()
        .range([height - padding, padding])
        .domain([bottomBound, top]);
      //x.domain([d3.min(data, (d)->parseFloat d.x), d3.max(data, (d)->parseFloat d.x)])
      //y.domain([0, (d3.max dataHist.map (i) -> i.length)])
      xAxis = d3.svg.axis().ticks(20).scale(xScale);
      yAxis = d3.svg
        .axis()
        .scale(yScale)
        .ticks(12)
        .tickPadding(0)
        .orient("right");
      lineGen = d3.svg
        .line()
        .x(function (d) {
          return xScale(d.x);
        })
        .y(function (d) {
          return yScale(d.y);
        })
        .interpolate("basis");
      //console.log("printing gaussian curve data")
      return _graph
        .append("svg:path")
        .attr("d", lineGen(curveData))
        .data([curveData])
        .attr("stroke", "black")
        .attr("stroke-width", 2.5)
        .attr("fill", "none");
    }
  }

  //#@gauss.drawKernelDensityEst(data, width, height, _graph, xAxis, yAxis, y, x)
  drawHist(_graph, data, container, gdata, width, height, ranges, bounds) {
    let $slider, arr, mean, median;
    //pre-set value of slider
    container.append("div").attr("id", "slider");
    $slider = $("#slider");
    // @bins = 5
    arr = data.map(function (d) {
      return parseFloat(d.x);
    });
    median = this.getMedian(arr);
    mean = this.getMean(arr);
    this.plotHist(
      this.bins,
      container,
      arr,
      _graph,
      gdata,
      x,
      height,
      width,
      data,
      median,
      mean,
      bounds
    );
    if ($slider.length > 0) {
      $slider
        .slider({
          min: 1,
          max: 10,
          value: this.bins,
          orientation: "horizontal",
          range: "min",
          change: function () {},
        })
        .addSliderSegments($slider.slider("option").max);
    }
    return $slider.on("slidechange", (event, ui) => {
      this.bins = parseInt(ui.value);
      //      tooltip.html()
      this.plotHist(
        this.bins,
        container,
        arr,
        _graph,
        gdata,
        x,
        height,
        width,
        data,
        median,
        mean,
        bounds
      );
      this.drawModelCurve(
        this.graphData,
        _graph,
        this.elem,
        container,
        gdata,
        width,
        height,
        ranges,
        bounds
      );
    });
  }
};
ModelerHist.inject(
  "app_analysis_modeler_kernel_density_plotter",
  "app_analysis_modeler_getParams"
);

module.exports = ModelerHist;


`  CalculateOptimalBinWidth: (data)  ->
      xMax = d3.max(data)
      xMin = d3.min(data)
      minBins = 4
      maxBins = 50
      results = []
      N for N in [minBins..maxBins] ->
        width = (xMax - xMax)/ N
        hist = []
        for x in data ->
          i = x - xMin / width
          if i >= N
            i = N -1
          y = xMin + width * i
          hist[y] += 1


        #compute mean and let
        sum = @getSum(hist)
        numOcc = hist.length
        mean = @getMean(sum, numOcc) # k
        letiance = @getletiance(data,  mean) # v

        C = (2 * mean - letiance) / width * 2

        results += [(hist, C, N, width)]



      optimal = d3.min(results)
      return optimal


results = []

for N in xrange(start, end):
	width = float(_max - _min) / N

	hist = defaultdict(int)
	for x in data:
		i = int((x - _min) / width)
		if i >= N:       # Mimicking the behavior of matlab.histc(), and
			i = N - 1    # matplotlib.hist() and numpy.histogram().
		y = _min + width * i
		hist[y] += 1

	# Compute the mean and let.
	k = fsum(hist[x] for x in hist) / N
	v = fsum(hist[x]**2 for x in hist) / N - k**2

	C = (2 * k - v) / (width**2)

	results += [(hist, C, N, width)]

optimal = min(results, key=itemgetter(1))

if 0: # if true, print bin-widths and C-values, the cost function.
	for (hist, C, N, width) in results:
		print '%f %f' % (width, C)

return optimal
`;
