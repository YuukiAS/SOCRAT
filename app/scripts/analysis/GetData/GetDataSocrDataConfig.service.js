...
module.exports =  class GetDataSocrDataConfig extends BaseModuleDataService {
  ...
  getNames() {
    return this.socrDatasets.map(function(dataset) {
      return {
  ...
  getUrlByName(datasetId) {
    ...
    return ((function() {
      ...
      ref = this.socrDatasets;
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        dataset = ref[i];
        if (datasetId === dataset.name.toLowerCase()) {
          results.push(dataset.url);
        }
      }
      return results;
    }).call(this)).shift();
